...
//Lop quan ly quan ly tat ca cac hoat dong cua dich vu ngan hang
class QuanLy{
	protected:
	public:
		...
		void THGiaoDich(string makh, int sotien, bool loaigiaodich){
				...
				if(KiemTra(makh) == 1){
					//neu laoi giao dich bang 1 thuc hien rut tien
					if(loaigiaodich == 1){
						for(int i = 0; i <= dskh.size(); i++){
							if(makh == dskh[i].makh){//makh nhap tu ban phim bang voi ma kh trong danh sach khach hang thi tien hanh gd
								if(dskh[i].sodu >= sotien)//so tien du lon hon so tien muon rut thi tien hanh gd
								{
									dskh[i].sodu -= sotien;
									f<<dskh.size()<<endl;
									for(int i = 0; i < dskh.size(); i++){
									...
									GiaoDich gd(makh, loaigiaodich, sotien);//tao mot gd moi va dua no vao vector giao dich
									dsTrans.push_back(gd);//them giao dich vao vector
									fi<<dsTrans.size()<<endl;//ghi vao file giao dich
		...
		//in sao ke,, cai nay chua hoan thanh
		void InSaoKe(){
